...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, X(dest) = Mr(src1 + imm, 4););
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, X(dest) = (src1 < imm)?1:0;); // 无符号小于立即数则置位
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, X(dest) = (src1 << imm);); // 逻辑立即左移
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, X(dest) = src1 + imm;); // 加立即数
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = X(1);); // 跳转并寄存器链接
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, X(dest) = SEXT(Mr(src1 + imm, 2), 16);); // 半字加载
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, X(dest) = Mr(src1 + imm, 2);); // 无符号半字加载
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, (src1 != src2)?(s->dnpc = s->pc + imm):(s->pc=s->pc);); // 不相等时分支
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, (src1 == src2)?(s->dnpc = s->pc + imm):(s->pc=s->pc);); // 相等时分支
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, (src1 >= src2)?(s->dnpc = s->pc + imm):(s->pc=s->pc);); // 大于等于时分支
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, (src1 < src2)?(s->dnpc = s->pc + imm):(s->pc=s->pc);); // 小于时分支
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, (src1 < src2)?(s->dnpc = s->pc + imm):(s->pc=s->pc);); // 无符号小于时分支
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, X(dest) = src1 + src2;); // X[rd] = X[rs1] + X[rs2], 忽略算术溢出
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, X(dest) = src1 - src2;); // X[rd] = X[rs1] - X[rs2], 忽略算术溢出
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, X(dest) = src1 * src2;); // X[rd] = X[rs1] * X[rs2], 忽略算术溢出
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, X(dest) = src1 / src2;); // X[rd] = X[rs1] / X[rs2], 忽略算术溢出
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, X(dest) = src1 % src2;); // X[rd] = X[rs1] % X[rs2]
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, X(dest) = (src1 < src2)?1:0;); // 无符号小于则置位
